...
class split_aggregation_sampling:
    def __init__(self, img_lr, patch_size, stride, magnification_factor, device, multiple_gpus=False):
        '''
        ...
        self.weight = self.gaussian_weights(patch_size*magnification_factor, patch_size*magnification_factor, batch_size, channels)
    ...
    def aggregation_sampling(self, model, model_name):
        ...
        for i,patch_lr in tqdm(enumerate(self.data_loader_patches_lr)):
            patch_sr = self.model(patch_lr[0])
            patch_sr = patch_sr[0].permute(1,2,0).detach().cpu().numpy()
            patch_sr *= 255
            patch_sr = patch_sr.astype('uint8')[:,:,0]
            Image.fromarray(patch_sr).save(f'{folder_name}/patch_{i}.png')

        for i,patch_lr in tqdm(enumerate(self.data_loader_patches_lr)):
            patch_sr = Image.open(f'{folder_name}/patch_{i}.png')
            patch_sr = transforms.ToTensor()(patch_sr).unsqueeze(0).to(self.device)
    ...
    def gaussian_weights(self, tile_width, tile_height, nbatches, channels):
            """
            ...
            return torch.tile(weights, (nbatches, channels, 1, 1))

    def prepare_data_loader(self,):
        dataset = get_data_patches_lr(self.patches_lr)
        if self.multiple_gpus:
            data_loader = DataLoader(dataset=dataset, batch_size=1, shuffle=False, sampler=DistributedSampler(dataset, shuffle=False))
        else:
...
def launch(args):
    ...
    aggregation_sampling = split_aggregation_sampling(img_lr, patch_size, stride, magnification_factor, device, multiple_gpus)

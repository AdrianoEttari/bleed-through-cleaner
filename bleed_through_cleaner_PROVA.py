...
from utils import otsu_thresholding, otsu_morphological_batch, get_data_inference
...
class bleed_through_cleaner:
    def __init__(self, image, models_folder_path, device) -> None:
    ...
    def page_extract(self, model_name):
        ...
        number_of_pixels = self.image.shape[-1]*self.image.shape[-2]
        scale = np.sqrt(target_pixels/number_of_pixels)

        input_image_rescaled = F.interpolate(self.image, scale_factor=scale, mode="bilinear", align_corners=False)
        ...
        mask_page = model_page_extractor(input_image_rescaled)

        page_filtered_image, otsu_mask_cleaned = otsu_morphological_batch(mask_page, scale, starting_img = self.image)
    ...
    def bleed_through_finder(self,
        ...
        if page_extraction_model_name:
            ...
            page_filtered_image_tensor = self.image
...
def get_max_dimensions(folder_path):
    """Find the maximum width and height from all images in a folder."""
...
def pad_image(image, target_height, target_width):
    """Pad image to match target dimensions while keeping it centered."""
    h, w = image.shape[:2]
    ...
    # Apply padding (black by default, change value=(255,255,255) for white padding)
    padded_image = cv2.copyMakeBorder(image, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(0, 0, 0))
    
    return padded_image

def process_images(input_folder, output_folder):
    """Find max dimensions and pad all images to that size."""
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # Get the max width and height from all images
    ...
    for filename in os.listdir(input_folder):
        if filename.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            if image is not None:
                ...
                cv2.imwrite(os.path.join(output_folder, filename), padded_image)

    print("Padding completed for all images!")


if __name__ == "__main__":
    input_folder = os.path.join("books","5d41_sannazaro_le_rime")
    output_folder = os.path.join("books","5d41_sannazaro_le_rime_PAD")
    if not os.path.exists(output_folder):
    ...
    dataset = get_data_inference(output_folder)
    ...
    for i, batch in tqdm(enumerate(data_loader)):
        ...
        cleaner = bleed_through_cleaner(batch, models_folder_path, device)
